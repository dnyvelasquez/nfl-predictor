...
**Scoring stays regular-season-only, on purpose**: `getEquipos()`, `getEquiposDe()` (used by `getParticipantesConPuntaje()`/tabla de puntajes), and `acumularPuntajesEnParticipantes()` are all hardcoded/defaulted to `etapa='regular'` — the 4 new playoff-stage grids are additive data that does not yet feed into any point calculation. Making Wild Card/Divisional/Conferencia/Super Bowl picks also score points would require reworking the scoring formula to attribute each of `equipos`' per-round counters (`pw`/`pd`/`pc`/`sb`) to that round's own assignment instead of a single assignment — that's a deliberate follow-up, not implemented here.
